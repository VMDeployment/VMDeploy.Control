...
$parentModule = 'VMDeploy.Control'
if (-not $ModuleName) { $ModuleName = 'VMDeploy.Control.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\VMDeploy.Control'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\VMDeploy.Control.psd1"
$prereqHash = @{
